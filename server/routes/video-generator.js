...
// ===== 火山引擎 API 配置 =====
// ⚠️ 如需更换 API Key 或 Endpoint，修改这里
const ARK_API_KEY = 'b421bf5d-e706-44cd-8ff5-55d596f6d1ae';
const ARK_ENDPOINT_ID = 'ep-20260330134019-8pxb4';
...
// ===== 本地任务存储（Supabase 不可用时的 fallback）=====
const localTasksFile = process.env.VERCEL ? '/tmp/vg_tasks.json' : path.join(__dirname, '..', 'vg_tasks.json');
...
async function uploadVideoToStorage(taskId, videoUrl) {
  ...
  try {
    ...
    await supabase.from('vg_tasks').update({
    ...
    if (result.status === 200) {
      ...
      if (data.status === 'succeeded' && data.content) {
        ...
        if (videoUrl) {
          ...
          if (supabase) {
            supabase.from('vg_tasks').update(updateData).eq('id', req.params.taskId).then(({ error }) => {
...
router.get('/tasks', async (req, res) => {
  ...
  try {
    ...
    if (error) {
      // Supabase 表可能不存在，回退到本地
      console.error('获取历史记录失败 (回退到本地):', error);
      const tasks = readLocalTasks();
      return res.json({ success: true, data: tasks });
    }
